...
<div class="content-header">
	<div class="container-fluid">
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1 class="m-0 text-dark">{{$feed->name}}</h1>
			</div><!-- /.col -->
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="{{route('feeds')}}">Feeds</a></li>
...
<form method="POST" action="{{route('feeds.store')}}">
    @csrf
<input type="hidden" name="update" value="{{$feed->id}}">
				...
				<div class="card-body">
					<div class="border border-primary my-2 p-2">
						<div class="form-group">
                            ...
                            <input type="text" class="form-control" value="{{$feed->name}}" name="enname" placeholder="English Feed Name.." required>
							</div>
						</div>
						<div class="form-group">
							<div class="input-group">
								...
								<input type="text" class="form-control" value="{{$arFeed->name}}" name="arname" placeholder="Arabic Feed Name.." required>
							</div>
						</div>
						<div class="form-group">
							<div class="input-group">
								...
								<input type="text" class="form-control" value="{{$trFeed->name}}" name="trname" placeholder="Turkish Feed Name.." required>
							</div>
						</div>
					</div>
						...
						<div class="form-group">
                            ...
                            <select name="entags[]" id="entags" class="form-control" multiple required>
                                @foreach ($tagsEn as $tag )
                                    <option {{(in_array($tag->name,explode(',',$feed->tags)))?'selected':''}}>{{$tag->name}}</option>
                                    @endforeach
								</select>
							</div>
						</div>
						<div class="form-group">
							<div class="input-group">
								...
								<select name="artags[]" id="artags" class="form-control" multiple required>
                                    ...
                                    <option {{(in_array($tag->name,explode(',',$arFeed->tags)))?'selected':''}}>{{$tag->name}}</option>
                                    @endforeach
								</select>
							</div>
						</div>
							...
							<div class="input-group">
								...
								<select name="trtags[]" id="trtags" class="form-control" multiple required>
                                    @foreach ($tagsTr as $tag )
                                        <option {{(in_array($tag->name,explode(',',$trFeed->tags)))?'selected':''}}>{{$tag->name}}</option>
                                    @endforeach
								</select>
							</div>
						</div>
					...
					<div class="border border-primary my-2 p-2">
						<div class="form-group">
							<div class="input-group">
								...
								<select name="city" id="city" class="form-control" required>
									...
									<option {{($feed->city == $city) ? "selected":""}}>{{$city}}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class='form-group'>
							<div class='input-group'>
								...
								<input type='email' class='form-control' value="{{$feed->email}}" name="email" placeholder='Feed Email..' required>
							</div>
						</div>
						<div class='form-group'>
							<div class='input-group'>
								...
								<input type='text' class='form-control' value="{{$feed->phone}}" name="phone" placeholder='Feed Phone..' required >
							</div>
						</div>
						<div class='form-group'>
							<div class='input-group'>
								...
								<input type='url' class='form-control' value="{{$feed->website}}" name="website" placeholder='Feed Website' required>
							</div>
						</div>
					...
					<div class="row text-center">
						<div class="col-lg-4">
							<div class="cropU">
								<div id="placeholder1">
                                    <img src="{{$feed->pics[1]}}" class="img-thumbnail" />
						...
						<div class="col-lg-4">
							<div class="cropU">
								<div id="placeholder2">
                                    <img src="{{$feed->pics[2]}}" class="img-thumbnail" />
                                    <div class="spinner-border text-primary loading" style="display:none" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
								</div>
						...
						<div class="col-lg-4">
							<div class="cropU">
								<div id="placeholder3">
                                    <img src="{{$feed->pics[3]}}" class="img-thumbnail" />
                                    <div class="spinner-border text-primary loading" style="display:none" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
								</div>
					...
					<div class="row mt-4 text-center">
						<div class="col-lg-4">
							<div class="cropU">
								<div id="placeholder4">
                                    <img src="{{$feed->pics[4]}}" class="img-thumbnail" />
                                    <div class="spinner-border text-primary loading" style="display:none" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
								</div>
						...
						<div class="col-lg-4">
							<div class="cropU">
								<div id="placeholder5">
                                    <img src="{{$feed->pics[5]}}" class="img-thumbnail" />
                                    <div class="spinner-border text-primary loading" style="display:none" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
								</div>
						...
						<div class="col-lg-4">
							<div class="cropU">
								<div id="placeholder6">
                                    <img src="{{$feed->pics[6]}}" class="img-thumbnail" />
                                    <div class="spinner-border text-primary loading" style="display:none" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
								</div>
				...
				<div class="card-body">
					<div class="card">
						<div class="card-body text-center">
							<div class="cropU">
								<div id="placeholder">
                                    <img src="{{$feed->cover}}" class="img-thumbnail" />
                                    <div class="spinner-border text-primary loading" style="display:none" role="status">
                                            <span class="sr-only">Loading...</span>
                                        </div>
								</div>
					...
					<div class="form-group">
						<div class="input-group">
							...
							<select name="category" id="category_id" class="form-control" required>
                                ...
                                <option value="{{$category->id}}" {{($feed->category_id == $category->id) ? "selected":""}}>{{$category->name}}</option>
                                @endforeach
							</select>
						</div>
						<hr>
					...
					<div class="row">
						...
						<div class="col-lg-6">
							<div class="form-group">
								<label>
									Rate:
									<input type="number"  value="{{$feed->rate}}" class="form-control" value="0" name="rate" >
								</label>
							</div>
						</div>
						<div class="col-lg-6">
